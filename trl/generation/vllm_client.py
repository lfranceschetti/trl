...
class VLLMClient:
    ...
    def load_lora_adapter(self, lora_name: str, lora_path: str) -> None:
        ...
        logger.info("Requesting LoRA adapter load: lora_name='%s', lora_path='%s'", lora_name, lora_path)
        response = self.session.post(url, json={"lora_name": lora_name, "lora_path": lora_path})
        response.raise_for_status()
        payload = response.json()
        logger.info(
            "LoRA adapter load acknowledged by vLLM server: lora_name='%s', lora_int_id=%s",
            payload.get("lora_name", lora_name),
            payload.get("lora_int_id"),
        )
